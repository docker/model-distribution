...
func (s *LocalStore) WriteManifest(hash v1.Hash, rm []byte, tags []string) error {
	manifest, err := v1.ParseManifest(bytes.NewReader(rm))
	if err != nil {
		return fmt.Errorf("parse manifest: %w", err)
	}
	if err := writeFile(s.manifestPath(hash), rm); err != nil {
	...
	entry := newEntryForManifest(hash, manifest)
	idx = idx.Add(entry)
